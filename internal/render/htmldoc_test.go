...
func TestHTMLDocGolden(t *testing.T) {
	src := readTestdata(t, "example-report.html")
	...
	assertGolden(t, got, "example-report.rendered.html")
}
...
func TestHTMLDocAnchorsInTheDOM(t *testing.T) {
	tests := []struct {
		...
		{"golden fixture", readTestdata(t, "example-report.html")},
		{"crlf", []byte("<p>windows\r\nline\r\nendings</p>\r\n<pre>a\r\nb</pre>\r\n")},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			out, err := HTMLDoc(tt.src)
			if err != nil {
				t.Fatalf("HTMLDoc: %v", err)
			}
			assertDOMOffsets(t, tt.src, out)
		})
	}
}
